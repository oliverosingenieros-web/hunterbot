"""Provider para Lidl (Búsqueda de productos)."""
...
@register
class LidlProvider(BaseProvider):
    """Provider para Lidl a través de Bing Search."""
    ...
    async def search(self, criteria: SearchCriteria) -> list[Item]:
        """Busca productos en Lidl usando Bing Search para saltar bloqueos."""
        query = criteria.query or ""
        if not query:
            return []

        search_query = quote_plus(f"site:lidl.es {query}")
        ...
        try:
            ...
            logger.error("Error buscando en Lidl: %s", e)
