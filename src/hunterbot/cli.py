...
def search_cmd(
    query: str | None = typer.Option(
        None, "--query", "-q", help="Término de búsqueda o palabra clave"
    ),
    location: str | None = typer.Option(
        None,
        "--location",
        "-l",
        help="Ubicación o zona geográfica (ej. Malaga, Madrid)",
    ),
    category: str | None = typer.Option(
        "real_estate",
        "--category",
        "-c",
        help="Categoría: real_estate, product, boat, other",
    ),
    provider: str | None = typer.Option(
        "all", "--provider", "-p", help="Provider específico o 'all'"
    ),
    price_min: float | None = typer.Option(None, "--min-price", help="Precio mínimo"),
    price_max: float | None = typer.Option(None, "--max-price", help="Precio máximo"),
    min_score: float = typer.Option(
        5.0, "--min-score", "-s", help="Puntuación mínima a mostrar (1-10)"
    ),
    project: str | None = typer.Option(
        None, "--project", help="Nombre del proyecto para agrupar alertas"
    ),
    config_path: Path | None = typer.Option(
        None, "--config", help="Ruta a config.yaml"
    ),
):
    ...
    async def _exec():
        ...
        try:
            with console.status(
                f"[bold cyan]Buscando oportunidades en {provider or 'todos los providers'}...[/bold cyan]"
            ):
    ...
    if not filtered:
        console.print(
            f"[yellow]No se encontraron oportunidades con score >= {min_score}[/yellow]"
        )
        return

    table = Table(
        title=f"🎯 Oportunidades encontradas ({len(filtered)} items)",
        show_header=True,
    ...
    for opp in filtered:
        ...
        detail_str = (
            f"{item.price_per_m2:.0f} €/m²"
            if item.price_per_m2
            else (f"{item.length_m} m" if item.length_m else "-")
        )
...
def opportunities_cmd(
    ...
    if not results:
        console.print(
            "[yellow]No hay oportunidades históricas almacenadas que cumplan el criterio.[/yellow]"
        )
        return

    table = Table(
        title="💎 Top Chollos del Historial", show_header=True, header_style="bold blue"
    )
...
def stats_cmd(
    ...
    console.print(
        Panel(panel_content, title="📊 Estadísticas de HunterBot", border_style="cyan")
    )
...
def export_cmd(
    output: Path = typer.Option(
        Path("oportunidades.csv"), "--output", "-o", help="Archivo CSV de salida"
    ),
    config_path: Path | None = typer.Option(
        None, "--config", help="Ruta a config.yaml"
    ),
        ...
        for it in items:
            writer.writerow(
                [
                    ...
                    it.location or "",
                    it.url,
                    it.last_seen.isoformat(),
                ]
            )

    console.print(
        f"[bold green]✅ Exportados {len(items)} items a {output}[/bold green]"
    )
...
def providers_cmd(
    ...
    table = Table(
        title="🔌 Providers Registrados y Estado",
        show_header=True,
